...
namespace polygon {
    ...
    template <> struct point_traits<Point> {
        ...
        static inline coordinate_type get(PointRef point, orientation_2d orient)
        {
    ...
    namespace graph_builder {
        ...
        Graph voronoi::operator()(MapRef map)
        {
            ...
            for (auto& e : vd.edges()) {
                ...
                for (double k = 0; k <= 1 and not flag; k += 0.2) flag = is_collision(map, k * p0 + (1 - k) * p1);

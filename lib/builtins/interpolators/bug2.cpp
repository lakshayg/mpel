...
namespace builtin {
    namespace interpolator {
        ...
        bug2::bug2(double step) { _step = step; }

        Path bug2::operator()(MapRef map, PathRef path)
        {
            // sanity checks
            for (auto& e : path) assert(not is_collision(map, e));
            assert(path.size() > 1);
            if (path.size() != 2) std::cerr << "Are you sure you wanted to use the bug2_interpolator?" << std::endl;

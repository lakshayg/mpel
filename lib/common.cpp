...
double distance(PointRef p, SegmentRef s)
{
    ...
    double cross = p.x * (s.p1.y - s.p0.y) - p.y * (s.p1.x - s.p0.x) + (s.p1.x * s.p0.y - s.p0.x * s.p1.y);
    return std::abs(cross / len);
}

double distance(SegmentRef s, PointRef p) { return distance(p, s); }
...
Path subdivide_path(PathRef path, double len)
{
    ...
    for (auto pt : p) {
        if (distance(pt, ans.back()) > len) ans.push_back(pt);
    }
    if (ans.back() != path.back()) ans.push_back(path.back());
    return ans;
}

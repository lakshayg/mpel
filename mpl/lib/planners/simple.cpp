...
vector<Point> SimplePlanner::find_path(const Point& start, const Point& goal,
                                       bool draw) {
	...
	Point current;
	auto cmp = [fCost](const Point& a, const Point& b) {
		return fCost.at<double>(a) > fCost.at<double>(b);
	};
	map<int,int> parent;
